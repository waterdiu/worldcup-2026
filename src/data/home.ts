import type { DailyHeroData, GroupFixtureData } from '../types/tournament';
import type { Locale } from '../i18n/content';
import { groupFixtures } from './groupFixtures';
import dailyHeroJson from './dailyHero.json';
import { formatMatchupLabel, formatTeamName, formatVenueName } from '../i18n/formatters';
...
const dailyHero = dailyHeroJson as DailyHeroData;
...
function buildGeneratedDailyHeroSlide(locale: Locale): HeroSlideData {
  const matchupLabel = `${dailyHero.homeTeam} vs ${dailyHero.awayTeam}`;
  const kickoffDate = new Date(dailyHero.kickoff);
  const localizedMatchup = formatMatchupLabel(matchupLabel, locale);

  return {
    id: `daily-hero-${dailyHero.matchId}`,
    type: 'match',
    eyebrow: locale === 'zh' ? '明日焦点战' : 'Tomorrow Spotlight',
    title: locale === 'zh' ? localizedMatchup.replace(/^[^\s]+ /, '').replace(/ vs [^\s]+ /, ' vs ') : matchupLabel,
    description:
      locale === 'zh'
        ? `${formatTeamName(dailyHero.homeTeam, locale)} 对阵 ${formatTeamName(dailyHero.awayTeam, locale)}，首页海报由每日任务自动更新。`
        : `${dailyHero.homeTeam} face ${dailyHero.awayTeam}. This homepage poster is generated by the daily update pipeline.`,
    ctaLabel: locale === 'zh' ? '进入比赛详情' : 'Open Match Detail',
    href: `/matches/${dailyHero.matchId}`,
    dateLabel: locale === 'zh' ? formatChineseDate(kickoffDate) : formatEnglishDate(kickoffDate),
    venueLabel: locale === 'zh' ? formatVenueName(dailyHero.venue, locale) : dailyHero.venue,
    matchupLabel,
    artworkUrl: dailyHero.poster,
    fallbackArtworkUrl: dailyHero.fallbackPoster,
    accent: 'daily'
  };
}
...
export function getHomepageHeroSlides(
  ...
  return [buildGeneratedDailyHeroSlide(locale)];
}
