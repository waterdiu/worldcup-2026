...
- The teams overview page uses an 8-column by 6-row compact grid on desktop so 48 teams do not leave an empty final row. The full team card must be the click target, not only the team name; cards only carry team name, group, confederation, and ranking with height compressed to content.
...
- Until `football-data-platform` publishes staff records in `core/team-staff.json`, `src/data/teamProfiles.ts` may provide `coach` and `coachAge` as page-level supplements; do not fabricate unconfirmed coaching staff.
