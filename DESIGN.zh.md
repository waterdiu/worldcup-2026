...
- 球队总览页使用紧凑自适应多列网格。桌面端不得固定为 3 列大卡片；卡片只承载队名、小组、赛区和排名，行高按内容压缩。
...
- 教练数据在 `football-data-platform` 的 `core/team-staff.json` 有数据前，允许由 `src/data/teamProfiles.ts` 提供 `coach` 和 `coachAge` 作为页面级补充；不能为未确认球队伪造教练组。
